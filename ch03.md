...
...
```
找到配置文件的位置后，可以使用Ctrl-C退出程序。一般来说，配置文件位于当前用户的主目录的.bitcoin数据目录下。系统不会自动创建这个文件的，需要手动从下面示例的这个链接地址（https://github.com/bitcoinbook/bitcoinbook/blob/develop/ch03.asciidoc#full_index_node ）复制粘贴过来。也可以使用自己的默认编辑器手动创建进行修改。
